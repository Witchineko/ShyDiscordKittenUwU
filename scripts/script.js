...
const ui = (function () {
    ...
    function playSound () {
        var audio = new Audio('sounds/h3ntai.mp3');
        audio.loop = false;
        audio.play();
    }
    ...
    function drow(html) {
        //draw html and call event loader
        let deck = gttingDeck();
        deck.innerHTML = "";
        deck.innerHTML = html;
        document.querySelector(
            "#player-name"
        ).innerHTML = `${current.gameType} matching `;
        const height = parseInt(
            getComputedStyle(document.querySelector(".card")).width.slice(0, -2)
        );
